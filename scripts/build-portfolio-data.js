...
function validateProject(project, index) {
  ...
  if (
    project.year &&
    (typeof project.year !== 'number' || project.year < 2000 || project.year > 2100)
  ) {
  ...
  if (project.accent && project.accent !== '' && !/^#[0-9A-Fa-f]{6}$/.test(project.accent)) {
    projectWarnings.push(
      `Accent color '${project.accent}' should be a valid hex color (e.g., #D29A84)`
    );
  }
  ...
  if (projectWarnings.length > 0) {
    warnings.push(
      `Project #${index + 1} (${project.id || 'unnamed'}): ${projectWarnings.join(', ')}`
    );
  }
